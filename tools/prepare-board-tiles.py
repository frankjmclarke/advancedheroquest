...
Run from any directory. Normal builds use the checked-in atlas and need no
...
def generate_features(out):
    ...
    rope.putdata([(r, g, b, 0 if b > r * 1.15 and b > g * 1.1 else 255)
                  for y in range(rope.height) for x in range(rope.width)
                  for r, g, b, a in [rgba[x, y]]])
    art = {
    ...
    for first, cols, rows in groups:
        for direction, rotation in enumerate(rotations):
            ...
            # Keep full RGB precision until identical patches are shared.
            tile.save(out / f'feature{old_id}.bmp')
...
def pack_shared_artwork(out):
    """Losslessly share identical 16x16 patches, including quarter turns.

    This automatically shares floors underneath different features, walls,
    symbols, and the repeated panels of oversized rooms. Full sprites exist
    only as build-time references in obj; only the atlas is embedded.
    """
    paths = {}
    for k, kind in enumerate(KINDS):
        for d, direction in enumerate('nsew'):
            paths[207 + k * 4 + d] = f'{kind}{direction}.bmp'
    for i in range(6):
        for d, direction in enumerate('nsew'):
            paths[231 + i * 4 + d] = f'room{i}{direction}.bmp'
    text = (ROOT / 'rsh/board-features.rc').read_text()
    paths.update({int(r): name for r, name in re.findall(r'(\d+) BITMAP "board/([^"\n]+)"', text)})
    block = 16
    patches, lookup, recipes, images = [], {}, [], []
    rotations = (None, Image.Transpose.ROTATE_90, Image.Transpose.ROTATE_180, Image.Transpose.ROTATE_270)
    for resource, filename in sorted(paths.items()):
        image = Image.open(out / filename).convert('RGB')
        assert image.width % block == image.height % block == 0
        start = len(recipes)
        rebuilt = Image.new('RGB', image.size)
        for y in range(0, image.height, block):
            for x in range(0, image.width, block):
                patch = image.crop((x, y, x + block, y + block))
                variants = [patch if turn is None else patch.transpose(turn) for turn in rotations]
                key, turn = min((v.tobytes(), k) for k, v in enumerate(variants))
                if key not in lookup:
                    lookup[key] = len(patches)
                    patches.append(variants[turn])
                index = lookup[key]
                assert index < 16384
                recipes.append(index * 4 + turn)
                restored = patches[index]
                if turn:
                    restored = restored.transpose(rotations[4 - turn])
                rebuilt.paste(restored, (x, y))
        assert rebuilt.tobytes() == image.tobytes(), filename
        images.append(dict(id=resource, width=image.width, height=image.height, offset=start,
                           file=filename, sha256=hashlib.sha256(image.tobytes()).hexdigest()))
    columns = 32
    atlas = Image.new('RGB', (columns * block, ((len(patches) + columns - 1) // columns) * block))
    for i, patch in enumerate(patches):
        atlas.paste(patch, ((i % columns) * block, (i // columns) * block))
    destination = ROOT / 'rsh/board'
    destination.mkdir(exist_ok=True)
    atlas.save(destination / 'atlas.bmp')
    header = ['/* Generated by tools/prepare-board-tiles.py. */',
              '#define BOARD_ATLAS_RESOURCE 701', f'#define BOARD_ATLAS_BLOCK {block}',
              f'#define BOARD_ATLAS_COLUMNS {columns}', '#define BOARD_IMAGE_ENTRIES { \\']
    header += [f'    {{{r["id"]}, {r["width"]}, {r["height"]}, {r["offset"]}}}, \\' for r in images]
    header += ['}', 'static CONST _UWORD board_patch_codes[] = {']
    header += ['    ' + ','.join(map(str, recipes[i:i+24])) + ',' for i in range(0, len(recipes), 24)]
    header += ['};', '']
    (ROOT / 'rsh/board-atlas.h').write_text('\n'.join(header), encoding='ascii')
    (ROOT / 'rsh/board-atlas.json').write_text(json.dumps(dict(block=block, columns=columns,
        patches=len(patches), images=images, codes=recipes), separators=(',', ':')) + '\n')
    (ROOT / 'rsh/board-features.rc').write_text('/* Shared board artwork. */\n701 BITMAP "board/atlas.bmp"\n')
    # Retire only the generated standalone sprites now replaced by the atlas.
    for filename in paths.values():
        obsolete = destination / filename
        assert obsolete.resolve().parent == destination.resolve()
        obsolete.unlink(missing_ok=True)
    print(f'Shared {len(recipes)} patches as {len(patches)} unique patches; atlas {atlas.width}x{atlas.height}.')
